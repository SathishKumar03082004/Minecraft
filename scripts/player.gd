	...
	if Input.is_action_just_pressed("left_click"):
		if ray_cast_3d.is_colliding():
			...
			if collider.has_method("destroy_block"):
				collider.destroy_block(ray_cast_3d.get_collision_point() - ray_cast_3d.get_collision_normal())
				
	if Input.is_action_just_pressed("right_click"):
		if ray_cast_3d.is_colliding():
			var collider = ray_cast_3d.get_collider()
			if collider.has_method("place_block"):
				collider.place_block(ray_cast_3d.get_collision_point() + ray_cast_3d.get_collision_normal(), 6)
